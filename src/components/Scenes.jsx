...
{objects.filter((object)=> object.sceneId === currentScene.id).map((object, index) => (
     <button className="objectButton" key={index} onClick={() => handleButtonClick()}>
        <img src={object.imgsrc} alt={object.name} style={{ maxWidth: '3vw', marginRight:"0.5vw "}} />{object.name}</button>
))}
